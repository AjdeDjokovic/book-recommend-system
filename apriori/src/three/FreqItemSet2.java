...
public class FreqItemSet2 {
	public static class FreqItemSet2Mapper extends Mapper<LongWritable, Text, Text, IntWritable>
	{
		...
		private HashSet<String> freq = new HashSet<String>();
		...
		/*
		 * input:					value:ISBN \t ISBN \t ISBN
		 * output:	key:ISBN;ISBN	value:one
		 */
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			for(int i = 0;i < length;i++)
				...
				{
					if(freq.contains(parts[i]) && freq.contains(parts[j]))
					{
	...
	/*
	 * input:	key:ISBN;ISBN	value:one
	 * output:	key:ISBN;ISBN	value:sum
	 */
